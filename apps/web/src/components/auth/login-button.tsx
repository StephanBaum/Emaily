...
/**
 * Microsoft/Outlook icon SVG component
 */
...
/**
 * LoginButton component for OAuth authentication with email providers.
 *
 * This client component handles sign-in via Google (Gmail) or Microsoft (Outlook).
 * It uses next-auth/react's signIn function to initiate the OAuth flow.
 */
export function LoginButton({
  ...
  const handleSignIn = async () => {
    ...
    try {
      ...
      // Error handling is managed by next-auth
      setIsLoading(false);
    }
  };

  const isGoogle = provider === "google";
  const Icon = isGoogle ? GmailIcon : OutlookIcon;
  const label = isGoogle ? "Connect Gmail" : "Connect Outlook";
